...
public class BlogServicesImpl implements BlogServices {
	...
	public List<Comment> searchOffensiveLanguageComments() throws ServicesException {
            try {
                List<Blog> blogs = blogDAO.loadAll();
                List<Comment> comentariosMalos = null;
                for (Blog b : blogs) {
                    List<Comment> comentarios = commentDAO.searchCommentsByBlogTitle(b.getTitle());
                    for (Comment c: comentarios){
                       comentariosMalos.add(c);
                    }
                }
                return comentariosMalos;
            } catch (PersistenceException ex) {
                throw new ServicesException("Search error:"+ex.getLocalizedMessage(), ex);
            }
		//throw new UnsupportedOperationException("Not supported yet.");
